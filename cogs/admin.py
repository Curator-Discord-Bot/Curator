...
from .utils.converter import GlobalTextChannel
...
class Admin(commands.Cog):
    ...
    @commands.group(name='reload', aliases=['update'], invoke_without_command=True)
    async def _reload(self, ctx, *, module):
        """Reloads a module."""
        try:
            if module == 'cogs.count':
                running_counts = self.bot.cogs['Count'].running_counts
                self.bot.reload_extension(module)
                self.bot.cogs['Count'].running_counts = running_counts
            else:
                self.bot.reload_extension(module)
        except commands.ExtensionError as e:
            await ctx.send(f'{e.__class__.__name__}: {e}')
    ...
    @commands.command()
    async def sudo(self, ctx, channel: Optional[GlobalTextChannel], who: Union[discord.Member, discord.User], *, command: str):
